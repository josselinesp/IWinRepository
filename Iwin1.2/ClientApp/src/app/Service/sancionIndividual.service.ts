...
@Injectable()
export class SancionIndividualService {
  ...
  guardarSancion(sancion: SancionIndividual): Observable<SancionIndividual> {
    let body = sancion;
    let headers = new Headers({ 'Content-Type': 'application/json' });
    console.log("PASA A POST")
    let options = new RequestOptions({ headers: headers });
    return this.http.post(this.url + 'api/sancionIndividual/', body, options).map(this.extractData);
